...
# styles.py may hide the collapse/expand arrow — this one line overrides it.
st.markdown("""
<style>
/* Force the sidebar open/close arrow button to always show */
...
/* Also ensure the header-level toggle is never hidden */
button[kind="header"] {
...
def sidebar():
    with st.sidebar:
        ...
        # ── NAV: use radio so page state survives sidebar open/close ──────
        # st.button() loses state when sidebar is closed; st.radio() persists.
        nav_items = [
        ...
        # Inject CSS to make radio look like the original button nav
        st.markdown("""
        <style>
        ...
        div[data-testid="stRadio"] > div > label {
            ...
            color: #ffffff;
            background: linear-gradient(135deg, #1a2a4a 0%, #162240 100%);
        ...
        div[data-testid="stRadio"] > div > label[data-checked="true"] {
            background: linear-gradient(135deg, #0044bb 0%, #0066ee 100%) !important;
            border-color: rgba(0,180,255,0.6) !important;
            color: #ffffff !important;
            box-shadow: 0 4px 20px rgba(0,100,255,0.45) !important;
            transform: translateX(3px);
        }
        ...
        # Sync session_state.page → radio index
        current_idx = nav_keys.index(st.session_state.page) \
                      if st.session_state.page in nav_keys else 0
        ...
        # Sync radio selection → session_state.page
